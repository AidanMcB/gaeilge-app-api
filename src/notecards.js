...
const getAllNotecards = (req, resp) => {
	pool.query('SELECT * FROM notecards ORDER BY id ASC', (error, results) => {
		if (error) {
			throw error;
		}
		const data = [];
...
const createNotecard = (req, resp) => {
	...
	pool.query(
		'INSERT INTO notecards (english_phrase, irish_phrase, color) VALUES ($1, $2, $3) RETURNING *',
		[englishPhrase, irishPhrase, color],
		(error, results) => {
			if (error) {
				throw error;
			}
